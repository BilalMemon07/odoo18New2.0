...
class account_payment(models.TransientModel):
    ...
    def _get_total_amount_in_wizard_currency_to_full_reconcile(self, batch_result, early_payment_discount=True):
        ...
        if self.source_currency_id == self.currency_id:
            ...
            if self.manual_currency_rate_active and self.manual_currency_rate:
                return (self.source_amount_currency / (1 / self.manual_currency_rate)), False
            else:
            ...
            if self.manual_currency_rate_active and self.manual_currency_rate:
                return abs(sum((aml.amount_residual * (1 / self.manual_currency_rate)) for aml in batch_result['lines']
                )), False
            else:
                return abs(sum(
                    comp_curr._convert(
            ...
            if self.manual_currency_rate_active and self.manual_currency_rate:
                return self.source_amount * (1 / self.manual_currency_rate), False
            else:
    ...
    def _create_payment_vals_from_wizard(self, batch_result):
        ...
        if self.manual_currency_rate_active and self.manual_currency_rate:
            conversion_rate = (1 / self.manual_currency_rate)
        else:
